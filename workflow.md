...
`input.external.wav` and `input.external.rawaudio`. Here, "raw audio" data is
consists in interleaved samples encoded as signed 16 bits little-endian
integers. For instance, we can play a sine wave which is generated by the
program `ffmpeg` with
...
"`!`": `a ! b` means that the output of operator `a` should be fed to operator
`b`. We refer the reader to the documentation of the library for more
information about it. Here, it can be passed in the argument labeled `pipeline`,
as expected. For instance, we can generate a sine wave (again) with
...
will play all the files in the `Music` directory. The playlists generally
contains one file per line, such as
...
As another example, we can base our decision on the metadata of the file as
follows:
...
more efficient in the case the file is distant because it is downloaded once for
all):
...
Also, streams in HLS format are quite different from the above (they consist of
a rolling playlist of short audio segments, see [there](#sec:HLS)) and are
...
then take the first line with `list.hd` and return a request from created from
it using `request.create`. As a variant, suppose that the next song to be played
...
\TODO{queue.push / queue.push.uri / pushing a playlist}
...
TODO: explain how to play one track only at a specified time...................
...
TODO: explain how to save the values with persistency
