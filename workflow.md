...
In order to input from a soundcard you should use functions such as `input.alsa`
or `input.pulseaudio` or `input.portaudio` depending on the library you want to
use for this: the first one is a little more efficient, because closer to the
hardware, and the second is more portable and widespread. For instance, you can
hear your voice with
...
A radio generally starts with a playlist, which is obviously a list of files to
be played. The `playlist`\index{playlist@\texttt{playlist}} operator does that:
it takes as argument either a playlist or a directory (in which case the
playlist will consist of all the files in the directory). For instance, the
script

```{.liquidsoap include="liq/playlist.liq" from=1}
```

will play all the files in the `Music` directory and the script

```{.liquidsoap include="liq/playlist2.liq" from=1}
```

will play all the files listed in the `playlist` playlist. The format of the
playlists generally consist in listing files, with one file per line, such as
...
software. By default, the files are played in a random order but this can be
changed with the `mode` parameter of `playlist` which can either be
...
its genre is "Rock" (remember that the metadata is encoded as an association
...
Liquidsoap to run an Icecast-compatible server, called
...
Again, the `track_sensitive` parameter controls whether a change of source only
occurs at track boundaries (when `true`, which is the default) or as soon as
possible.
...
```{.liquidsoap include="liq/switch2.liq" from=6 to=6}
```
...
A tricky, but quite common, situation is the following one: suppose that we have
a source `music`, which is playing music, and that we want to play one
advertisement from an `ad` source (typically, a playlist containing all our
advertisements) around noon, without interrupting the current track. One would
typically write a script such as

```{.liquidsoap include="liq/switch-once.liq" from=3 to=-1}
```

but this is not really good: if the track from the music source ends at 12h01
then no ad will be played. In order to accommodate for this, we are tempted to
widen the time predicate and replace the second line with

```liquidsoap
    ({12h00-12h15}, ad),
```

Well, this is not good either: if the music source ends at 12h01, we now hear an
ad as expected, but we actually continuously hear ads for 14 minutes instead of
hearing only one. In order to fix this, we are tempted to use the `once`
operator and change the line to

```liquidsoap
    ({12h00-12h15}, once(ad)),
```

This is not good either: `once(ad)` plays only one track from `ad`, but during
the whole execution of the script. This means that our script will only work as
expected on the first day, where we correctly hear one ad, but on the second and
following days we will hear no ad because one ad has already been played. The
right solution, consist in using the `predicate.once` function: it takes a
predicate `p` as argument, and returns a predicate which is true only once each
time `p` is continuously true. This means that `predicate.once({12h00-12h15})`
is a predicate which is true once between 12h00 and 12h15. This is exactly what
we were looking for, and the right script is

```{.liquidsoap include="liq/switch-once2.liq" from=3 to=-1}
```

As a variant, if we wanted to play 3 ads, we could replace the second line by

```liquidsoap
    (predicate.at_most(3, {12h00-12h15}), ad),
```

where `predicate.at_most` is similar to `predicate.once`, but is true a given
number of times instead of only once.
...
where `jingles` is a playlist containing all our jingles. The usual setups for
jingles are the following ones:

- we can play a jingle between two music tracks
...
```{.liquidsoap include="liq/jingles-add.liq" from=3 to=-1}
```
...
for instance. Here is the recipe (1200 sec = 20 min):
...
TODO: jingle at fixed hour with `predicate.activates` / `predicate.at_most` for limiting

TODO: we don't want to use `once`...
